...
- [ ] **Debounce cancel on unmount** (`components/BPFViewer.tsx:61-77`)
...
- [ ] **Debounce utility missing cancel** (`utils/debounce.ts:36-49`)
...
- [ ] **FractionDesign crash on empty stages** (`components/designs/FractionDesign.tsx:68-71`)
...
- [ ] **Orphaned promise in fetchWithTimeout** (`services/BPFService.ts:243-248`)
...
- [ ] **Hardcoded entity display names** (`index.ts:315-318`)
...
- [ ] **Hardcoded track color `#E1E1E1`** (`components/designs/LineDesign.tsx:28`, `GradientDesign.tsx:28`)
...
- [ ] **Hardcoded default colors in manifest** (`ControlManifest.Input.xml:143-148`)
...
- [ ] **Magic numbers in design components** (all 8 design files)
...
- [ ] **Mobile aria-labels missing** (design components)
...
- [ ] **Accessibility tests too shallow** (`__tests__/a11y/accessibility.test.tsx`)
...
- [ ] **ErrorBoundary uses inline styles** (`components/ErrorBoundary.tsx:45-78`)
...
- [ ] **Unsafe `any` casts** (`index.ts:169,202`, `configValidation.ts:52,84`, `debounce.ts:32`)
...
- [ ] **Unused `_entityName` parameter** (`services/BPFService.ts:80`)
...
- [ ] **Missing JSDoc on public types** (`types/index.ts`)
...
- [ ] **Default design loaded lazily** (`components/designs/index.tsx:21-28`)
...
- [ ] **Missing OData pagination** (`services/BPFService.ts:189`)
...
- [ ] **No memoization on BPFRow** (`components/BPFRow.tsx`)
...
- [ ] **PCF build pipeline** (`.github/workflows/ci.yml`)
...
- [ ] **Solution packaging scripts** (`package.json`)
...
- [ ] **Missing `.cdsproj` / Solution project**
...
- [ ] **Console logging in production** (`index.ts`, `services/BPFService.ts`)
...
- [ ] **Inconsistent error logging** (`services/BPFService.ts:112,211,311,376,435`)
...
- [ ] **`escapeODataValue` silent on falsy input** (`utils/sanitize.ts:106-112`)
...
- [ ] **README coverage section outdated on each release**
